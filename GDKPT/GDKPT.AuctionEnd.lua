...
local function UpdateSummaryPanel(WonAuctionsSummaryPanel)
    ...
    -- Calculate what's left: raid cut minus (what's owed + adjustments)
...
function GDKPT.AuctionEnd.HandleAuctionEnd(auctionId, GDKP_Pot, itemID, winningPlayer, finalBid)
    ...
    if winningPlayer == UnitName("player") and finalBid > 0 then
        -- Try to get item info, but use fallback if not available
        local itemName, itemLink = GetItemInfo(itemID)
    
        -- Fallback: check if we have the name stored on the row
        ...
        table.insert(
            GDKPT.Core.PlayerWonItems,
            {
                ...
                bid = finalBid,
                auctionId = auctionId,
                amountPaid = 0,  -- Track partial payments
                timestamp = time()
            }
        )
