...
FavoriteFrame:SetPoint("LEFT", GDKPT.UI.AuctionWindow, "LEFT", -100, 0)
...
FavoriteFrame:SetFrameLevel(GDKPT.UI.AuctionWindow:GetFrameLevel() + 2)
...
function GDKPT.Favorites.ToggleFavorite(itemLink)
    ...
    if GDKPT.Favorites.FavoriteFrame and GDKPT.Favorites.FavoriteFrame:IsVisible() then
        GDKPT.FavoritesUI.Update()
    end

    GDKPT.Favorites.UpdateAuctionRowVisuals(itemID)
...
function GDKPT.Favorites.IsFavorite(itemID)
    return GDKPT.Core.PlayerFavorites[itemID] ~= nil
end

-- Gets the favorite data {link, maxBid}
function GDKPT.Favorites.GetFavoriteData(itemID)
    return GDKPT.Core.PlayerFavorites[itemID]
end

-------------------------------------------------------------------
-- Visual Updating for Favorites
-------------------------------------------------------------------


-- Updates the star/glow for any matching auction rows
function GDKPT.Favorites.UpdateAuctionRowVisuals(itemID)
    ...
    for auctionID, frame in pairs(GDKPT.Core.AuctionFrames) do
        if frame.itemID == itemID then
            local isFav = GDKPT.Favorites.IsFavorite(itemID)
...
function GDKPT.Favorites.UpdateAllAuctionRowVisuals()
     for auctionID, frame in pairs(GDKPT.Core.AuctionFrames) do
        if frame.itemID then
            GDKPT.Favorites.UpdateAuctionRowVisuals(frame.itemID)
        end
     end
end
...
function GDKPT.Favorites.FilterByFavorites()
...
function GDKPT.FavoritesUI.Update()
    ...
    local ScrollFrame = GDKPT.Favorites.FavoriteScrollFrame
    local ScrollContent = GDKPT.Favorites.FavoriteScrollContent
...
function GDKPT.Favorites.CheckAutoBid(auctionID, itemID, minNextBid, highBidderName, itemLink)
    ...
    local favoriteData = GDKPT.Favorites.GetFavoriteData(itemID)
...
function GDKPT.Favorites.RemoveFavoriteWhenAuctionWon(itemID, winnerName)
    local myName = UnitName("player")
    ...
    if winnerName == myName then
        ...
        if favoriteData then
            ...
            GDKPT.Favorites.ToggleFavorite(favoriteData.link)
        end
    end
end
...
function GDKPT.Favorites.CheckLootedItemForFavorite(itemLink)
    if not itemLink then return end
...
GDKPT.Favorites.FavoriteFrame = FavoriteFrame
GDKPT.Favorites.FavoriteScrollFrame = FavoriteScrollFrame
GDKPT.Favorites.FavoriteScrollContent = FavoriteScrollContent
